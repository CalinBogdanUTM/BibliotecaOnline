     ...
     public class AdminAPI
     {
          ...
          public BAddResp UpdateBookInfoAction(int bookId, BookInfo updatedBookData)
          {
              using (var bookContext = new BookContext()) // Assuming you have a BookContext class that inherits from DbContext
              {
                  ...
                  if (book == null)
                  {
                      return new BAddResp { Status = false, StatusMsg = "Book not found" };
                  }
       ...
       internal UEmailResp UpdateUserEmailAction(int userId, string newEmail)
          {
              using (var userContext = new UserContext())
              {
                  ...
                  if (user == null)
                  {
                      return new UEmailResp { Status = false, StatusMsg = "User does not exist" };
                  }

                  // Check if the new email is already registered
                  if (userContext.Users.Any(u => u.Email == newEmail && u.Id != userId))
                  {
                      return new UEmailResp { Status = false, StatusMsg = "Email already registered" };
                  }
